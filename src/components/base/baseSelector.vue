...
<script>
    ...
    export default {
            ...
            selected() {
                ...
                return this.dates.findIndex(date => {
                    return (
                        ...
                        (this.typeSelect == "year" || date.getMonth() == this.beginDate.getMonth())
                    );
                });
            },
            dates() {
                ...
                if (this.typeSelect == "month") {
                    ...
                    dates = [...Array(2).keys()].map(
                        year =>
                            new Date(new Date().getFullYear() + year, 0, 1)
                    );
                }
                return dates
            }
        ...
        watch: {
            ...
            // typeSelect(val) {
            //     if (val == "month") {
            //         this.dates = [...Array(13).keys()].map(
            //             month =>
            //                 new Date(new Date().getFullYear(), new Date().getMonth() + month, 1)
            //         );
            //     } else if (val == "year") {
            //         this.dates = [...Array(2).keys()].map(
            //             month =>
            //                 new Date(new Date().getFullYear(), new Date().getMonth() + month, 1)
            //         );
            //     }
